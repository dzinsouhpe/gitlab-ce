...
SERVICENAME = "sssd"
SERVICEPATH = "/lib/systemd/system/sssd.service"
SERVICEDESC = "User Auth"
REGATTEMPTS = 5

UbuntuSampleSSSDConf = '''
[domain/default]
...
def epic_config_get_decoded(param):
    if param == 'connections.secrets.extAuth.data.ca_cert':
        return base64.b64decode(ConfigMeta.getWithTokens(param.split('.'))).decode('utf-8')
...
def get_epic_context_params():
    log("Fetching misc context info from ECP...")
...
def is_ldap_secured():
    ...
    if auth_auth_type == u'LDAP' or auth_auth_type == u'Active Directory':
        auth_tls_enabled = epic_config_get('connections.secrets.extAuth.data.security_protocol')
        try:
...
def get_epic_auth_params(context_params, sssd_domain, transmitted_domain=None):
    ...
    log("Fetching auth info from ECP...")
    auth_server_uri = ""
    try:
    ...
    security_protocol = get_guaranteed_param_from_config('security_protocol')
    verify_peer = get_optional_param_from_config('verify_peer')
    if security_protocol == 'ldaps' and verify_peer == 'true':
        sssd_domain.set_option('ldap_tls_reqcert', 'hard')
        auth_ca_cert = get_optional_param_from_config('ca_cert')
    else:
...
#    else:
    ...
    mem_of_cluster_grp = get_members_param_from_config('groups')
    mem_of_tenant_grp = get_members_param_from_config('tenantGroups')
    mem_of = '::::'.join(filter(None,[mem_of_cluster_grp, mem_of_tenant_grp]))
    if mem_of:
        if len(mem_of) > 0:
...
def update_cert(auth_ca_cert, sssd_domain, transmitted_domain=None):
    ...
    if auth_ca_cert is not None:
        ...
        sssd_domain.set_option('ldap_tls_cacert', cert_path)
    else:
...
def start_sssd():
    ...
    try:
        subprocess.check_call(["systemctl", "enable", "sssd.service"])
        with open(SERVICEPATH, 'r+') as f:
            file = f.readlines()
            for i, line in enumerate(file):
               if line.startswith('Type=notify'):
                   file[i] = file[i].strip() + '\nRestart=always\n'
            f.seek(0)
            for line in file:
                f.write(line)
